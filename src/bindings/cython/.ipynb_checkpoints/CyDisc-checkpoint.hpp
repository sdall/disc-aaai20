...
// py::dict
// desc_impl(Dataset<S> dataset, std::vector<size_t> labels, size_t min_support)
// {
...
//         // dataset = std::move(c.data.data);
//         // labels = std::move(c.data.template col<0>());
...
//     auto dict = pyutils::describe_partitions(x, y, min_support, is_sparse, use_higher_precision_float);
...
auto cy_desc_from_matrix(PyObject* dataset, PyObject* labels, int min_support, bool is_sparse, bool use_higher_precision_float)
{
    ...
    auto dataset_11 = py::reinterpret_borrow<py::array_t<size_t>>(dataset);

...
cdef extern from "<bindings/cython/CyDisc.hpp>" namespace "sd::disc":
    ...
    cdef cppclass DefaultTrait
    cdef cppclass itemset[T]:
        void insert(int x)
        void clear()
    cdef cppclass Dataset[T]:
        void insert(itemset[T] x)
    dict desc[T](Dataset[T] x, vector[int] y, int ms, int high_precision_float)
    dict disc[T](Dataset[T] x, int ms, float alpha, int high_precision_float)
...
cdef Dataset[tag_sparse] to_dataset_sparse(xs):
    ...
    for x in xs: 
        t.clear()
        for i in x: t.insert(i)
        data.insert(t) # why is this not working?
    return data

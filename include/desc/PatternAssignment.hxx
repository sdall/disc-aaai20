                          ...
                          Interface&&         f = {})
{
    ...
    if (counter)
    {
        ...
        auto row = c.confidence.extent(0);
        c.confidence.resize(sd::layout<2>({row + 1, conf.size()}));
        for (size_t i = 0; i < conf.size(); ++i) { c.confidence(row, i) = conf[i]; }
...
bool find_assignment_impl_first(Composition<Trait>& c, const Candidate& x, const Config&)
{
    ...
    if (x.score <= 0 || !c.models[0].is_allowed(x.pattern)) return false;
...
bool find_assignment(Composition<Trait>& c,
                     ...
                     Interface&&         f = {})
{
    if (c.data.num_components() == 1) { return find_assignment_impl_first(c, x, cfg); }
    else
    {

...
#include <disc/desc/PatternsetMiner.hxx>
...
//     Component<Trait> c;
//     c.data  = std::move(x);
//     elapsed_time_fit = timeit([&] { discover_patterns_generic(c, cfg, std::forward<MinerType>(m)); });
//     return c;
// }

// template<typename Trait, typename MinerType = DefaultPatternsetMinerInterface>
// auto desc(Dataset<typename Trait::pattern_type> x, Config cfg, std::optional<std::vector<size_t>> const& y = std::nullopt, MinerType && m = {})
// {
...
//     void fit(Dataset<pattern_type> x, std::optional<std::vector<size_t>> const& y = std::nullopt) 
//     {
//         if (y && y->size() != 0)
//         {
...
//     void characterize(Dataset<pattern_type> x, const Dataset<pattern_type>& summary, std::optional<std::vector<size_t>> const& y = std::nullopt) 
//     {
//         if (y && y->size() != 0)
//         {
...
//             return std::reduce(std::execution::par_unseq, begin(x), end(x), float_type{}, [&](auto l, const auto& xi) 
//             {
...
//     auto log_likelihood(Dataset<pattern_type> x, std::optional<std::vector<size_t>> const& y = std::nullopt) 
//     {
//         if (std::holds_alternative<Component<trait_type>>(state))
//         {
//             const auto& c = std::get<Component<trait_type>>(state);
...
//             return std::reduce(std::execution::par_unseq, r.begin(), r.end(), float_type{}, [&](auto l, const auto& xi) 
//             {
//                 return l - c.models[label(xi)].log_probability(point(xi));
//             });
//         }
